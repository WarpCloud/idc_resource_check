...
def init_excel(path):
    ...
    wb = xlwt.Workbook()
    sheet = wb.add_sheet("storage")
    value = [["Namespace", "PV_NAME", "Size"]]
...
if __name__ == '__main__':
    ...
    excel_path = os.path.join(dest_file_dir, "idc_resource.xls")
        ...
        if os.path.isfile(path):
            with open(path, 'r') as f:
                for line in f:
                    ...
                    pv_name = line.strip('\n').split()[0]
                    storage_disk = line.strip('\n').split()[1]
                    values.append([ns, pv_name, storage_disk])
